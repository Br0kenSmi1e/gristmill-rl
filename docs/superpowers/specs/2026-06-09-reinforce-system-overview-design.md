...
scalar step           -> authoritative one-sample reference semantics
...
Does not define the neural architecture, training optimizer, or a required
standalone scalar rollout implementation.
...
2. Rust/PyO3 row environment wrappers for `RewriteStateRow` and
   `ActionSpaceRow`.
3. Scalar policy sampling and scoring functions with `jax.vmap` coverage.
4. Rectangular row storage.
5. Width-1 row rollout execution covering scalar reference cases.
6. Multi-sample row stepping with scalar-equivalence tests.
7. Row scoring and deterministic fake-advantage REINFORCE loss.
...
- Equivalence tests showing row stepping matches scalar reference semantics and
  scalar Rust environment calls.
...
- A width-1 row rollout can train through recomputed target/action logp.
